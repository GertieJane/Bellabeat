...
ggplot(data = daily_activity_v2, aes(x = factor(1), y = TotalSteps)) +
  geom_boxplot() +
  theme_minimal()+
  labs(title="Daily Activity Total Steps")
```

In daily_activity_v2, I checked TotalSteps and SedentaryHours to see how often zero activity was recorded and looked for above-average values. There are 72 rows with zero steps, distance, activity, and a full day of sedentary activity. I would like to know why there are so many days like this. Does the tracker record activity while it is charging? It is doubtful that the tracker needs to charge for a full day, but maybe some users had battery issues. If a user forgets to wear their device for a day, is this how the data is recorded? Is this normal usage, a product error, or a user error? It could be issues with data collection or migration. I would look for these answers before deleting the data, but these are important issues to raise, especially concerning user behavior.  
...
The daily_sleep summary reveals that there are no zero values in the columns. It is unclear what the column TotalSleepRecords is. The average is 1.12 records per row, but it does not seem helpful, so I filtered it out to make daily_sleep_v2. I summarized daily_sleep_v2 to see the number of days recorded and averages per user in daily_sleep_summary. There is very inconsistent usage of the sleep recording feature. Out of 31 possible days and 33 users, the average number of sleep days recorded is 17 days, with only 24 users. Any analysis made with this data frame will be weak, but it would be interesting to see if there is any correlation between sleep and daily activity.     
...
# Boxplot for total_records
ggplot(data = hourly_intensity_summary, aes(x = factor(1), y = total_records)) +
  geom_boxplot() +
  theme_minimal()+
  labs(title="Hourly Intensities Records")
...
In hourly_intensity, I changed the character date to date time and noticed this set lists every hour of the day and how much intensity was recorded during those hours per user. There are 33 users, no duplicates, and an average of 28 days recorded. I had a feeling that this data frame had the same usage from users as the daily_actity data frame, but to make sure, I made a boxplot and summary of days recorded and the number of records. The days recorded, average days, and box plot are all very similar to daily_activity. It will be interesting to see which hours of the day are more active for users.   
...
# Boxplot for total_records
ggplot(data = hourly_steps_summary, aes(x = factor(1), y = total_records)) +
  geom_boxplot() +
  theme_minimal()+
  labs(title="Hourly Steps Records")
